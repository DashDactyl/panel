...
class FindAssignableAllocationService
{
    ...
    {
        if (!config('DASHDACTYL.client_features.allocations.enabled')) {
        ...
        // server.
        /** @var \DASHDACTYL\Models\Allocation|null $allocation */
    ...
    {
        $start = config('DASHDACTYL.client_features.allocations.range_start', null);
        $end = config('DASHDACTYL.client_features.allocations.range_end', null);
        ...
        /** @var \DASHDACTYL\Models\Allocation $allocation */
        $allocation = $server->node->allocations()
            ->where('ip', $server->allocation->ip)
            ->where('port', $port)
            ->firstOrFail();
