...
use DASHDACTYL\Repositories\Eloquent\RecoveryTokenRepository;
use DASHDACTYL\Exceptions\Service\User\TwoFactorAuthenticationTokenInvalid;
...
class ToggleTwoFactorService
{
     ...
     * @throws \DASHDACTYL\Exceptions\Service\User\TwoFactorAuthenticationTokenInvalid
     */
    public function handle(User $user, string $token, bool $toggleState = null): array
    {
        $secret = $this->encrypter->decrypt($user->totp_secret);

        $isValidToken = $this->google2FA->verifyKey($secret, $token, config()->get('DASHDACTYL.auth.2fa.window'));
