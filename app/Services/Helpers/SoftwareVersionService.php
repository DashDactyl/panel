...
namespace DASHDACTYL\Services\Helpers;
...
use DASHDACTYL\Exceptions\Service\Helper\CdnVersionFetchingException;

class SoftwareVersionService
{
    public const VERSION_CACHE_KEY = 'DASHDACTYL:versioning_data';
    public const GIT_VERSION_CACHE_KEY = 'DASHDACTYL:git_data';
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://DASHDACTYL.io/discord';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config('DASHDACTYL.cdn.cache_time', 60)), function () {
            try {
                $response = Http::get(config('DASHDACTYL.cdn.url'));
